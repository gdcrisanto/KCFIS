<?php
  ...
  if (isset($_POST['add_installment_submit'])){
    ...
    if (empty($dateReceived) || empty($receiptNo) || empty($amountReceived) || empty($modeOfPayment)) {
      header("Location: ../add_installment.php?acctid=".$acctinfoID."&error=emptyfields");
      exit();
    }
    else {
      ...
      if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("Location: ../add_installment.php?error=sqlerror1");
        exit();
      }
      else {
        ...
        header("Location: ../view_installment_list.php?acctid=".$acctinfoID."&add_installment=success");
        exit();
      }
    ...
    header("Location: ../add_installment.php");
    exit();
